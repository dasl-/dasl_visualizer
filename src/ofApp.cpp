...
void ofApp::update(){
    ...
    mouseForces.update(deltaTime, leftScaledVol, rightScaledVol);
...
void ofApp::audioIn(float * input, int bufferSize, int nChannels){
    ...
    for (int i = 0; i < bufferSize; i++){
        leftAudio[i]	= input[i*2]*0.5;
        rightAudio[i]	= input[i*2+1]*0.5;
        
        lCurVol += leftAudio[i] * leftAudio[i];
        rCurVol += rightAudio[i] * rightAudio[i];
        numCounted+=1;
    }
    
    //this is how we get the mean of rms :)
    lCurVol /= (float)numCounted;
    rCurVol /= (float)numCounted;
    
    // this is how we get the root of rms :)
    lCurVol = sqrt( lCurVol );
    rCurVol = sqrt( rCurVol );
    
    float currentVolumeWeight = 1.0;
    leftSmoothedVol *= 1 - currentVolumeWeight;
    rightSmoothedVol *= 1 - currentVolumeWeight;
    leftSmoothedVol += currentVolumeWeight * lCurVol;
    rightSmoothedVol += currentVolumeWeight * rCurVol;
//    bufferCounter++;
